...
//special trimming for ellipse arc
    ...
    for(int ii=0; ii<trimSol.getNumber(); ii++) { //find closest according ellipse angle
        ...
        if( !ii ||  fabs( fmod( ias[ii] - am, 2*M_PI)) < fabs( fmod( ia -am, 2*M_PI)) ) {
            ia = ias[ii];
            is = trimSol.get(ii);
        }
    }
    ...
    if(RS_Math::isSameDirection(getAngle1(),getAngle2(),RS_TOLERANCE_ANGLE)
        ...
        setAngle1(ia);
        setAngle2(ia2);
        double da1=fabs(fmod(getAngle1()-am,2*M_PI));
        double da2=fabs(fmod(getAngle2()-am,2*M_PI));
        if(da2<da1) {
            std::swap(is,is2);
        }

    } else {
        double dia=fabs(fmod(ia-am,2*M_PI));
        double dia2=fabs(fmod(ia2-am,2*M_PI));
        ...
        if( da_min < ai_min ) {
            ...
            if ( RS_Math::isAngleBetween(ia,getAngle1(),getAngle2(), isReversed()) &&
                ...
                da1=fabs(fmod(getAngle1()-am,2*M_PI));
                da2=fabs(fmod(getAngle2()-am,2*M_PI));
            }
...
bool RS_Ellipse::isArc() const{
    ...
    using std::isnormal;
#endif
    return isnormal(getAngle1()) || isnormal(getAngle2());
}
/**
...
void RS_Ellipse::moveRef(const RS_Vector& ref, const RS_Vector& offset) {
    ...
    for(size_t i=0; i< 2 ; i++){
        if ((ref-foci.at(i)).squared()<1.0e-8) {
			...
			RS_Vector focusNew=foci.at(i) + offset;
            //move focus
            //auto&& center = getCenter() + offset*0.5;
			RS_Vector center = getCenter() + offset*0.5;
            RS_Vector majorP;
...
void RS_Ellipse::draw(RS_Painter* painter, RS_GraphicView* view, double& patternOffset) {
    ...
    for(unsigned short i=0;i<4;i++)
	{
		...
		RS_VectorSolutions vpIts=RS_Information::getIntersection(static_cast<RS_Entity*>(this), &line, true);
        ...
        foreach(RS_Vector vp, vpIts.getVector())
		{
            ...
            if( fabs( fmod(ap2 - ap1, M_PI) ) < RS_TOLERANCE_ANGLE) continue;
