...
bool RS_Arc::createLeadLineSideData(const RS_Vector& start, const RS_Vector& mouse,  RS_LineData *dt) {
	...
	if (distToCenter > data.radius) {
		//先看是否有满足的端点, 满足的切点，然后求最近的那个带你就ok了
		RS_VectorSolutions allVec;

		dv = getNearestEndpoint(mouse);//先加入最近合法端点
		RS_Line line(NULL, RS_LineData(start, dv));
		RS_VectorSolutions vec = RS_Information::getIntersection(&line, this, true);
		if (vec.getNumber() == 1)//上面传入true，表示只线段和弧只有一交点，即是满足条件的线，也就是说线终点就是dv
		{
			allVec.push_back(dv);
		}

		RS_VectorSolutions sol = getTangentPoint(start);//再加入合法最近切点
...
// 		RS_Vector po(allVec.getClosest(mouse));//获得最近点
// 
	...
	RS_LineData *dt1, RS_LineData *dt2, RS_ArcData *arcDt)
{
	...
	//求圆
		...
		{ //圆弧2 竖直切
			//RS_Vector direction = (start - data.center).prep();//正交向量
		...
		//确定弧数据
		double extAng1 = center.angleTo(vp);//切点角度
		double extAng2 = center.angleTo(interPoint1);//垂足角度
...
void RS_Arc::calculateBorders() {
	...
	//< ++ by liu.y.c 2016.07.14 默认圆弧都在XY平面内
...
//special trimming for ellipse arc
    ...
    double aMin=getAngle1();
    double aMax=getAngle2();
    if(isReversed()) qSwap(aMin, aMax);//arc两端点角度
    const double angleMouse=getArcAngle(mousePoint);//鼠标位置角度
    RS_Vector vp=trimSol.at(0);
    double da=fabs(fmod(getArcAngle(vp) - angleMouse, 2.*M_PI)); //每个交点和鼠标位置的夹角
