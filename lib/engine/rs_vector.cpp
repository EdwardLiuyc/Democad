...
RS_Vector::RS_Vector(Point3D p3D) {
...
double RS_Vector::angle() const {
    //atan2,返回的是 -pi 到 pi之间 不包括-pi      所以要调整到0 - 2pi之间
    return RS_Math::correctAngle(atan2(y,x)); // (x,y)和x正轴间夹角
//    double ret = 0.0;
//    double m = magnitude();
//
//    if (m>1.0e-6) {
...
double RS_Vector::xP(const RS_Vector& v1)
{
	return this->magnitude() * v1.magnitude() * sin(v1.angle() - this->angle());
}
