...
public:
	...
	void initWithEntityManager(EntityManager *eM);
	virtual CAMDataLink *getCamListReference(){return NULL;};
...
//                const QString& newName);
	...
	void MoveToTop(RS_Entity* en);//Modifiy @zhang.y.q 用于移动entity
	void MoveToBottom(RS_Entity* en);//Modifiy @zhang.y.q 用于移动entity
	void MoveTo(int form,int to);//Modify @zhang.y.q
	QList<RS_Entity*> &getEntities();//获得entities
...
private:
    int entIdx;
    bool autoDelete;		// 容器析构时，是否删除实体内存，默认为true
	bool m_bMutex;			// 互斥量，所有需要对entIdx变量的值进行改变的函数, 必须时m_bMutex= false，才能进行对值的修改操作
};
