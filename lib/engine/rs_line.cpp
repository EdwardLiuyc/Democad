...
bool RS_Line::createMidLeadLineData(const RS_Vector& coord1, RS_LineData *dt )
{
	if (dt == NULL)
	{
		return false;
...
bool RS_Line::createLeadLineData(const RS_Vector& coord, RS_LineData *dt) {
	if (dt == NULL)
	{
		return false;
	}
	...
	if( a < RS_TOLERANCE2) {
		...
		vpc = data.startpoint + direction*RS_Vector::dotP(vpc,direction)/a;//找投影点
		if(! vpc.isInWindowOrdered(minV,maxV) ){//点不在线上
			return false;
		}
	}

	*dt = RS_LineData(coord, vpc);
	return true;

}


bool RS_Line::createLeadLineSideData(const RS_Vector& start, const RS_Vector& mouse,  RS_LineData *dt) {
	if (dt == NULL)
	{
		return false;
	}

// 	RS_Vector ending = getNearestEndpoint(mouse, NULL);
// 	*dt = RS_LineData(start, ending);

// 	RS_Vector ending = getNearestPointOnEntity(mouse);
// 	*dt = RS_LineData(start, ending);

	RS_Vector rs_Bng = data.startpoint;
	RS_Vector rs_End = data.endpoint;
...
int RS_Line::createLeadArcLineData(const RS_Vector& start, const RS_Vector& mouse, double rd, bool tp,
	RS_LineData *dt1, RS_LineData *dt2, RS_ArcData *arcDt)
{
	...
	RS_Vector direction = data.endpoint-data.startpoint;
	RS_Vector vpc       = start-data.startpoint;
	double a            = direction.squared();
	...
	{
		vpc = data.startpoint + direction*RS_Vector::dotP(vpc,direction)/a;//找投影点
		if(! vpc.isInWindowOrdered(minV,maxV) )
		{
			return 2;		//点不在线上直接返回
		}
	}
	...
	{ //圆弧1  斜切
		...
		vp = sol.getClosest(mouse); //切点

		*dt2 = RS_LineData(start, vp); //获得线数据
		dt1->reset();
	...
	{ //圆弧2 竖直切
		...
		vp = sol.getClosest(mouse);//获得最近切点

		*dt2 = RS_LineData(vp +(start - center), vp); //获得线数据
		//确定dt1数据
	...
	//确定弧数据
	double ang1 = center.angleTo(vp);//切点角度
	double ang2 = center.angleTo(vpc);//垂足角度
...
RS_Vector RS_Line::getFarthestEndpoint(const RS_Vector& coord,
	double* dist)const {
		double dist1((data.startpoint-coord).squared());
		double dist2((data.endpoint-coord).squared());
...
RS_Vector RS_Line::getFarthestPointOnEntity(const RS_Vector& coord,
	bool onEntity, double* dist, RS_Entity** entity)const {
    ...
    RS_Vector RS_Line::getMiddlePoint()const
{
    ...
    RS_Vector RS_Line::getNearestMiddle(const RS_Vector& coord,
        ...
        double l=dvp.magnitude();//求线段长
        if( l<= RS_TOLERANCE) {
            //line too short
            return const_cast<RS_Line*>(this)->getNearestCenter(coord, dist);
        }
        RS_Vector vp0(getNearestPointOnEntity(coord,true,dist));//找到实体上离坐标最近的点
...
LC_Quadratic RS_Line::getQuadratic() const
{
    ...
    RS_Vector dvp=data.endpoint - data.startpoint;
...
//prepare trimming for multiple intersections
    ...
    RS_Vector vp0=trimSol.getClosest(trimCoord,NULL,0);
    ...
    RS_Vector dvp1=vp1 - trimCoord;
    RS_VectorSolutions sol1;
    for(size_t i=0; i<trimSol.size(); i++){
        RS_Vector dvp2=trimSol.at(i) - trimCoord;
...
void RS_Line::draw(RS_Painter* painter, RS_GraphicView* view, double& patternOffset) 
{
	...
	//< add by zhou.g.x 将上下指定R中的字母R绘制到图形上
	...
	//< 如果不是所有的端点都在可视串口内，对端点做处理，加入可视端点
    if( endPoints.size() < 2 )
	{
		//< 建立一个XY平面上的临时LINE，否则与边界可能无交点，导致放大显示区域时实体不显示
		RS_Vector tmpStartPoint = getStartpoint();
		RS_Vector tmpEndPoint   = getEndpoint();
		tmpStartPoint.z = 0.;
		tmpEndPoint.z   = 0.;
		RS_Line tmpLine( NULL, RS_LineData( tmpStartPoint, tmpEndPoint) );
        ...
        for(unsigned short i = 0; i < 4; ++i )
		{
            ...
            RS_VectorSolutions vpIts = RS_Information::getIntersection(reinterpret_cast<RS_Entity*>(&tmpLine)
				                                                       , &line
																	   , true);
			if( vpIts.size() == 0) 
				continue;
    ...
    if( (endPoints[0] - getStartpoint()).squared() > (endPoints[1] - getStartpoint()).squared() ) 
		std::swap( endPoints[0], endPoints[1]);  //< 容器内部的互换

    RS_Vector pStart( view->toGui(endPoints.at(0)) );
    RS_Vector pEnd( view->toGui(endPoints.at(1)) );
    RS_Vector direction = pEnd - pStart;
    //if(/*isHelpLayer(true) */ false && direction.squared() > RS_TOLERANCE){
	...
	//< 若未选中，则直接画直线; 或者线段的长度很小，也是直接画
    if (( !isSelected() && getPen().getLineType() == RS2::SolidLine ) || length <= RS_TOLERANCE ) 
	{
    ...
    if (isSelected()) 
        ...
        pat = view->getPattern(getPen().getLineType()); //< 根据画笔选择类型
