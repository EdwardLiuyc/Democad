...
lead(false), m_leadType(RS2::LeadUnknown),m_pLeadLinkEntity(NULL), m_bLeadLink(false){

    this->parent = parent;
    init();
	//this->pen.setColor(RS_Color(255, 255, 255)); 
}
 ...
 */
void RS_Entity::init() {
	m_graphNum=0;//Modify @zhang.y.q
	//m_entityType=QString("none");//@zhang.y.q
    resetBorders();
 ...
 */
/*对所有实体进行唯一编号*/
void RS_Entity::initId(long int iid ) {
