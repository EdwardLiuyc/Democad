...
/**
 * Gets the farthest end point to the given coordinate.
 *
 * @param coord Coordinate (typically a mouse coordinate)
 *
 * @return the coordinate found or an invalid vector
 ...
 * container.
 */
RS_Vector RS_Information::getFathestEndpoint(const RS_Vector& coord,
        double* dist) const {
    return container->getFarthestEndpoint(coord, dist);
}
...
/**
 * Gets the farthest point to the given coordinate which is on an entity.
 *
 * @param coord Coordinate (typically a mouse coordinate)
 * @param dist Pointer to a double which will contain the
 *        measured distance after return or NULL
 ...
 * container.
 */
RS_Vector RS_Information::getFarthestPointOnEntity(const RS_Vector& coord,
        bool onEntity,
        double* dist,
        RS_Entity** entity) const {

    return container->getFarthestPointOnEntity(coord, onEntity, dist, entity);
}
...
RS_VectorSolutions RS_Information::getIntersection(RS_Entity* e1,
    ...
    if (onEntities &&
           /* (! (e1 -> isHelpLayer() || e2 -> isHelpLayer() ))
            && */(
                (e1 -> getMin().x > e2 -> getMax().x + tol)
                || (e1 -> getMax().x + tol < e2 -> getMin().x)
                || (e1 -> getMin().y > e2 -> getMax().y + tol)
                || (e1 -> getMax().y + tol < e2 -> getMin().y)
                )
            ) {
            return ret;
    }
...
// 	const auto&& qf2=e2->getQuadratic();
	...
	LC_Quadratic qf1=e1->getQuadratic();
	LC_Quadratic qf2=e2->getQuadratic();

    ret=LC_Quadratic::getIntersection(qf1,qf2);
    RS_VectorSolutions ret2;
    for(int i=0;i<ret.getNumber();i++) {
        const RS_Vector & vp=ret.get(i);
        if ( ! ret.get(i).valid) continue;
        if (onEntities==true) {//忽略不在实体上的点
            //ignore intersections not on entity
            if (!(
                        (e1->isPointOnEntity(vp, tol)) &&
                        (e2->isPointOnEntity(vp, tol))
                        )
                    ) {
                continue;
            }
 ...
 *         on the contour.
 */
//判断一个点是否在一个轮廓内，RS_EntityContainer来描述一个轮廓
