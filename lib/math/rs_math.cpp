...
/*
    ...
    if(fabs( fmod(correctAngle(a2 - a1 ) , 2.*M_PI)) < RS_TOLERANCE_ANGLE) return true;
...
*/
	...
	//是否受顺逆影响，占时不管 
	a = RS_Math::correctAngleTo2P(a);
	a1 = RS_Math::correctAngleTo2P(a1);
	a2 =  RS_Math::correctAngleTo2P(a2);
...
double RS_Math::correctAngle(double a){
    return M_PI + fmod(a - M_PI, 2*M_PI);  //2Pi还是2PI而不是0
}

//把负角度调整到0-2PI 包括负的
double RS_Math::correctAngleTo2P(double a){
	double b = a;
	b = correctAngle(b); //先调整到 0-2Pi 0 -  -Pi
	if (b >= 0.0)
	{
		return b;
	} else {
...
//0-2Pi把角度调整到-pi - pi  给dxf的弧转到绘图数据的弧用
double RS_Math::correctAngleToP(double a) {
	double b = correctAngle(a); //先调整到 0-2Pi  0 -  -Pi
...
double RS_Math::getAngleDifference(double a1, double a2, bool reversed) {
    if(reversed)
        return M_PI + fmod(a1 -a2 -M_PI, 2*M_PI);
    else
        return M_PI + fmod(a2 -a1 -M_PI, 2*M_PI);
}
...
//        std::cout<<"cubic: one root: "<<ans[0]<<std::endl;
    ...
    if(discriminant>0) {
		...
		std::vector<double>  r = quadraticSolver(ce2);
...
//        }
    ...
    if ( fabs(r)< 1.0e-75 ) {
		...
		std::vector<double> r=cubicSolver(cubic);
	...
	std::vector<double> r3= cubicSolver(cubic);
    ...
    if (r3.size()==1) { //one real root from cubic
		...
		std::vector<double> r1=quadraticSolver(ce2);
        if (r1.size()==0 ) {
            ce2[0]=	sqrtz0;
            ce2[1]=0.5*(p+r3[0])-0.5*q/sqrtz0;
...
+
 (-(c^2 g^2) + b c g h - a c h^2 - b^2 g i + 2 a c g i + a b h i - a^2 i^2) y^4
	...
	std::vector<double> roots=quarticSolverFull(qy);
    if(RS_DEBUG->getLevel()>=RS_Debug::D_INFORMATIONAL){
...
//                        std::cout<<"x^2 +("<<ce2[0]<<")*x+("<<ce2[1]<<")==0"<<std::endl;
			...
			std::vector<double> xRoots=quadraticSolver(ce2);
...
{
    RS_VectorSolutions ret;
    const std::vector<double> *p0 = &(m[0]);
    const std::vector<double> *p1 = &(m[1]);
