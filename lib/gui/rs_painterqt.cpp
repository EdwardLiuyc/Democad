...
void RS_PainterQt::drawArc(const RS_Vector& cp, double radius,
                           ...
                           bool reversed) {
    if(radius<=0.5) {
        drawGridPoint(cp);//半径过小，画点表示这个弧
    } else {
#ifdef __APPL1E__
                drawArcMac(cp, radius, a1, a2, reversed);
#else
        QPolygon pa;  //类似用 QVector<QPoint>描述一个多边形
        createArc(pa, cp, radius, a1, a2, reversed);//根据参数创建点
        drawPolyline(pa);//画出这些点，实际就是个弧
#endif
    }
}
...
void RS_PainterQt::drawText(const QPointF &position, const QString &text)
{
