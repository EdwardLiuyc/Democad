...
//    if (RS_DIALOGFACTORY==NULL) return;
    DIALOGMEDIATOR->requestSnapDistOptions(distance, snapMode.snapDistance);
    DIALOGMEDIATOR->requestSnapMiddleOptions(middlePoints, snapMode.snapMiddle);
}
...
RS_Vector RS_Snapper::snapPoint(QMouseEvent* e) {
    ...
    if( ! snapSpot.valid ) {
        ...
        if(snapMode.snapFree){
            const RS_Vector & ds=mouseCoord - snapSpot;
            const RS_Vector & grid=graphicView->getGrid()->getCellVector()*0.5;
...
//手动设置snap点，并且绘制悬浮层的那个用来捕获snap点的十字
    ...
    if (!finished && snapSpot.valid) {
        ...
        if (snapCoord.valid) {
                        ...
                        if (showCrosshairs==true) {
                            if(false/*graphicView->isGridIsometric()*/) {//isometric crosshair
                                RS2::CrosshairType chType=graphicView->getGrid()->getCrosshairType();
